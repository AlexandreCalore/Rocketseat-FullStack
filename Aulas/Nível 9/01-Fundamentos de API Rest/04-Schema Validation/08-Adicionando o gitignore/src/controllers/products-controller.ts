...
class ProductsController {
  ...
  create(request: Request, response: Response){
      ...
      const bodySchema = z.object({ //Aqui estamos validando o corpo da requisição, verificando se o nome é uma string e o preço é um número
        ...
        .min(6, {message: "O nome do produto deve ter no mínimo 6 caracteres"})
        .trim(), //Validações no geral para esse campo

        price: z
        .number({required_error: "O preço do produto é obrigatório"}) //Poderiamos usar o .Nullish() para permitir receber vazio
